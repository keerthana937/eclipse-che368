...
RUN sudo pip install --no-cache-dir notebook numpy pandas scipy scikit-learn statsmodels matplotlib && \
	sudo pip install --no-cache-dir ipython sklearn regex Werkzeug Flask Flask-Cors &&\
	sudo pip install --no-cache-dir Keras tensorflow tensorflow-estimator
